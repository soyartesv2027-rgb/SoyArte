
<!-- ===== FOOTER ===== -->
<footer class="footer-soyarte">
    <div class="container py-5">
        <div class="row gy-4">
            <!-- LOGO Y DESCRIPCION -->
            <div class="col-lg-4 col-md-6">
                <h2 class="footer-logo">
                    Soy Arte 🎨
                </h2>
                <p class="footer-description">
                    Un espacio creado para inspirar, conectar y compartir
                    el talento artístico en todas sus formas.
                </p>
            </div>

            <!-- ENLACES -->
            <div class="col-lg-2 col-md-6">

                <h5 class="footer-title">
                    Navegación
                </h5>

                <ul class="footer-links">

                    <li>
                        <a href="sobre-nosotros.php">
                            Sobre nosotros
                        </a>
                    </li>

                    <li>
                        <a href="contacto.php">
                            Contáctanos
                        </a>
                    </li>

                    <li>
                        <a href="apoyanos.html">
                            Apóyanos
                        </a>
                    </li>

                </ul>

            </div>

            <!-- CONTACTO -->
            <div class="col-lg-3 col-md-6">

                <h5 class="footer-title">
                    Contacto
                </h5>

                <p class="footer-contact">
                    <i class="fa-solid fa-envelope me-2"></i>
                    soyartesv2027@gmail.com
                </p>

            </div>

            <!-- REDES -->
            <div class="col-lg-3 col-md-6">

                <h5 class="footer-title">
                    Síguenos
                </h5>

                <div class="social-icons">

                    <a href="#">
                        <i class="fa-brands fa-facebook-f"></i>
                    </a>

                    <a href="https://www.instagram.com/soyarte._">
                        <i class="fa-brands fa-instagram"></i>
                    </a>

                    <a href="#">
                       <i class="fa-brands fa-x-twitter" style="color: rgb(0, 0, 0);"></i>
                    </a>

                    <a href="https://youtube.com/@soyarte_sv?si=8A1vyXwRi_OW3DI4">
                        <i class="fa-brands fa-youtube"></i>
                    </a>

                </div>

            </div>

        </div>

        <hr class="footer-divider">

        <div class="text-center">

            <p class="footer-copy">
                © 2027 Soy Arte — Todos los derechos reservados
            </p>

        </div>

    </div>

</footer>