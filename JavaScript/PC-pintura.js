...
const startX = centerX - Math.sin(angle) * travelDistance;

const startY = centerY - Math.cos(angle) * travelDistance;
...
function drawText() {
  ...
  const reveal = Math.min(1, Math.max(0, (progress - 0.35) * 5));

  ctx.globalAlpha = reveal;

  // TITULO

  ctx.font = `bold ${Math.min(canvas.width * 0.12, 150)}px Brush Script MT`;
  ...
  ctx.fillText("SoyArte", canvas.width / 2, canvas.height / 2 - 20);

  // SUBTITULO

  ctx.font = `italic ${Math.min(canvas.width * 0.04, 55)}px Georgia`;

  ctx.fillStyle = "rgba(0,0,0,0.72)";

  ctx.fillText("Pintura", canvas.width / 2, canvas.height / 2 + 70);
...
function drawPaint() {
  ...
  for (let i = -380; i <= 380; i += 14) {
    const wave =
      Math.sin(i * 0.04 + progress * 12) * 18 + Math.cos(i * 0.02) * 10;
  ...
  for (let i = 380; i >= -380; i -= 14) {
    const wave =
      Math.cos(i * 0.05 + progress * 12) * 18 + Math.sin(i * 0.03) * 10;
...
function createParticles() {
  ...
  if (pos.y > canvas.height / 2 - 160 && pos.y < canvas.height / 2 + 160) {
    for (let i = 0; i < 5; i++) {
      particles.push({
...
function drawBrush() {
  ...
  const wood = ctx.createLinearGradient(-250, 0, 0, 0);
  ...
  const metal = ctx.createLinearGradient(0, -35, 70, 35);
  ...
  const bristles = ctx.createLinearGradient(70, 0, 220, 0);
  ...
  for (let i = 0; i < 28; i++) {
    ...
    ctx.moveTo(80 + i * 5, -25 + Math.random() * 50);

    ctx.lineTo(220, -15 + Math.random() * 30);
